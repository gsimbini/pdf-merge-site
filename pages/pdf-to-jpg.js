...
export default function PdfToJpgPage() {
  ...
  async function handleConvert() {
    ...
    try {
      setProcessing(true);
      setMessage('Rendering PDF page to JPG…');
      ...
      canvas.toBlob(
        (blob) => {
          if (!blob) {
            setMessage('Failed to create JPG image from this page.');
            setProcessing(false);
            return;
          }
          ...
          a.href = url;
          a.download = `${baseName}-page-${pageNum}.jpg`;
          ...
          setMessage(
            `Done! Exported page ${pageNum} to JPG. Check your downloads.`
          );
          setProcessing(false);
        },
        'image/jpeg',
  ...
  return (
    <>
      <Head>
        <title>PDF to JPG (Single Page) - SimbaPDF</title>
        <meta
          name="description"
          content="Export a single page from a PDF as a JPG image directly in your browser."
        />
        {/* PDF.js from CDN */}
        <script
...
<div style={{ marginTop: "0.75rem" }}>
  <ProBadge />
        ...
        <main className="main">
          <section className="tool-section">
            <h2>PDF to JPG (Single Page)</h2>
            <p>
              Export a single page from your PDF as a JPG image. Choose the page
              number, and this tool will render that page into a high-quality JPG.
            </p>
  ...
  onClick={() => document.getElementById('pdf-to-jpg-file-input')?.click()}
>
  <p>
...
{file && (
  <div style={{ marginTop: '1rem', fontSize: '1.1rem', color: '#333' }}>
    <strong>Selected:</strong> {file.name} ({formatBytes(file.size)})
            ...
            >
              {processing ? 'Processing…' : 'Convert page to JPG and download'}
            </button>
